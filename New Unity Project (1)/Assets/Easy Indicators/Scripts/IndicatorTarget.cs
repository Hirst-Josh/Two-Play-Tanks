...
public class IndicatorTarget : MonoBehaviour
{
    ...
    public void UpdateIndicator()
    {
        ...
        else
        {
            ...
            if ((ForceOffScreen || IndicatorViewer.TrackOffScreen) && IPanel.OffScreen != null)
            {
                ...
                {
                    //  If the off-screen indicator is within distance threshold, then do stuff...  
                    if (CheckDisableOnDistanceTeam2(distanceFromViewer) == false)
                    {
                        //  Enables the off screen indicator
                        if (!isOffScreenEnabled)
                            ToggleOffScreen(true);
                ...
                {
                    //  If the off-screen indicator is within distance threshold, then do stuff...  
                    if (CheckDisableOnDistanceflag(distanceFromViewer) == false)
                    {
                        //  Enables the off screen indicator
                        if (!isOffScreenEnabled)
                            ToggleOffScreen(true);
                ...
                {
                    //  If the off-screen indicator is within distance threshold, then do stuff...  
                    if (CheckDisableOnDistancePowerUp(distanceFromViewer) == false)
                    {
                        //  Enables the off screen indicator
                        if (!isOffScreenEnabled)
                            ToggleOffScreen(true);
    ...
    private bool CheckDisableOnDistanceflag(float distance)
    {
        //  Check if indicator will disable on distance and check if distance is over the distance threshold
        if (viewer.DisableOnDistanceflag && distance > viewer.DisableDistanceflag)
            return true;
        return false;
    }

    private bool CheckDisableOnDistanceTeam2 (float distance)
    {
        //  Check if indicator will disable on distance and check if distance is over the distance threshold
        if (viewer.DisableOnDistanceteam2 && distance > viewer.DisableDistanceteam2)
            return true;
        return false;
    }

    private bool CheckDisableOnDistancePowerUp(float distance)
    {
        //  Check if indicator will disable on distance and check if distance is over the distance threshold
        if (viewer.DisableOnDistancePowerup && distance > viewer.DisableDistancePowerUp)
            return true;
        return false;
    }
